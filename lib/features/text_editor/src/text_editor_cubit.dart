...
class TextEditorCubit extends Cubit<TextEditorState> {
  ...
  Future<void> publish() async {
    ...
    for (Element e in imageTags) {
      ...
      if (!src.startsWith('http')) {
        emit(state.copyWith(
            ...
            imageUploadError: const Wrapped.value(null)));
        ...
        if (_imageUploadCache[src] != null) {
          ...
          try {
            if (state.compressImages &&
                supportedImageResizeFormats.contains(imageType.toLowerCase())) {
              final img.Image? image = (await (img.Command()
                        ..decodeNamedImage(src, file))
                      .executeThread())
                  .outputImage;
              if (image == null) {
                ...
                if ((isSquare && width > 1280) ||
                    width > height && width > 1280 ||
                ...
                if (imageType == 'jpg' || imageType == 'jpeg') {
                  ...
                  await cmd.executeThread();
                  log('encode no jpg');
                  encodedImage = cmd.outputImage != null
                      ? img.encodeNamedImage(src, cmd.outputImage!)
                      : null;
                }
                log('output bytes: ${encodedImage?.length}');

                if (encodedImage != null &&
                    encodedImage.length < initialLength) {
                  file = encodedImage;
                }
              }
            }
          ...
          try {
            ...
            emit(state.copyWith(
                ...
                imageUploadError: Wrapped.value(e),
    ...
    if (state.imageUploadError == null) {
      ...
      emit(state.copyWith(
        finalHtml: htmlContent.body!.innerHtml,
        textHtml: '',
      ));
    }
  }
  ...
  void setStagePreview(String html, double maxWidth, bool isEditing) async {
    emit(state.copyWith(
        stage: TextEditorStage.preview, processingTextHtml: true));
    ...
    if (state.cardUrl == '' &&
      ...
      newCardUrl =
          _extractUrl(htmlContent.body!.innerHtml)?.replaceAll('&amp;', '&');
    } else {
  ...
  void setCardUrl([String? newUrl]) {
    emit(state.copyWith(
        cardUrl: _extractUrl(newUrl)?.replaceAll('&amp;', '&') ?? ''));
  }

  void switchWrapImagesWithLinks([bool? newValue]) {
    final value = newValue ?? !state.wrapImagesWithLinks;
    emit(state.copyWith(
        wrapImagesWithLinks: value, hideImages: !value ? false : null));
  }
